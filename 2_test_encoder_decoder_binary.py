...
# This class is instantiated to be testing_batch_generator
#
...
class BatchGenerator(keras.utils.Sequence):
    def __init__(self, testing_batch=True, B=30, T=20):
    ...
    # There is only one instance of class BatchGenerator. At the end of each testing epoch, on_epoch_end() will be automatically called by the system
    def on_epoch_end(self):  # set indices after each epoch
    ...
    def __grab_and_process_single_img(sequence_num, agent_idx, step_idx=None):  # private method
        ...
        if step_idx is not None:  # grab single x image
            base_dir = '../0_Honglu_data_preprocessing/microscopic_domain/microscopic_image_samples_T_steps_binary/'
        ...
        # [3] threshold to binary {0, 1}, for both single x(t) image and ground truth y_img
        # img_ds_th = np.where(img_ds > (255/2.), 1, 0)  # for model_index 0, 1, 2, 3
        img_ds_th = np.where(img_ds > (255/2.), 0, 1)  # for model_index 4
    ...
    # a data point is x_sequence without y_img, per agent; a batch is an instance
    def __process_curr_data_point(self, sequence_num, curr_agent_idx):  # a private method
        ...
        for t in range(self.__T):  # t denotes step index within a sequence
            ...
            if x_chs_dsrows_dscols is not None:  # an x image is grabbed
                ...
                # print(np.array_equal(list_x_one_sample[-1], list_x_one_sample[-2]))
    ...
    def __getitem__(self, batch_index):
        ...
        # for an instance (Batch), prepare testing batch: only x_BS_T_Ch_Row_Col, without y_BS_Ch_Row_Col, by concatenating np
        # array vertically from empty where BS = batch size, total number of valid agents in one instance
        list_x_all_points_in_a_batch = []
        list_y_all_points_in_a_batch = []

        # all x_sequences in current instance
        x_base_dir = '../0_Honglu_data_preprocessing/microscopic_domain/microscopic_image_samples_T_steps_binary/' + 'instance' + str(sequence_num) + '/'
        ...
        return x_BS_T_Ch_Row_Col  # do not return (x_BS_T_Ch_Row_Col, y_BS_Ch_Row_Col)
...
if __name__ == '__main__':
    ...
    dir_model = './models/micro_wo_disentangle_binary_v{}.h5'.format(model_index)
    # model = keras.models.load_model(dir_model)  # for model_index 0, 1, 2
    # model = keras.models.load_model(dir_model, custom_objects={'weighted_binary_crossentropy_v0': weighted_binary_crossentropy_v0})  # for model_index 3
    model = keras.models.load_model(dir_model, custom_objects={'soft_dice_loss': soft_dice_loss})  # for model_index 4
    ...
    raw_pred_y_ALL_Ch_Row_Col = model.predict_generator(testing_batch_gen,
                                                        ...
                                                        verbose=0)
    tac()
    print('shape of raw_pred_y_ALL_Ch_Row_Col = {}'.format(raw_pred_y_ALL_Ch_Row_Col.shape))
    print('')

    dir_raw_y = './predicted_labels_binary' + '/v{}'.format(model_index)
    ...
    for seq_idx in range(seq_starting_index, seq_starting_index+num_seq):
        ...
        dir_y = './predicted_labels_binary' + '/v{}'.format(model_index) + '/instance' + str(sequence_num)
