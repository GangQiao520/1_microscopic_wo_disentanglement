...
class BatchGenerator(keras.utils.Sequence):
    ...
    def __grab_and_process_single_img(sequence_num, agent_idx, step_idx=None):  # private method
        ...
        # for model index 2
        # if step_idx is not None:  # for single x image: preserve original binary value
        #     img_ds_th = np.where(img_ds > (255/2.), 1, 0)
        # else:  # for ground truth y_img: swap binary value
        #     img_ds_th = np.where(img_ds > (255/2.), 0, 1)

        # img_ds_th = np.where(img_ds > (255 / 2.), 1, 0)  # for model_index 0, 1, 3
        img_ds_th = np.where(img_ds > (255/2.), 0, 1)      # for model_index 4, 5
...
if __name__ == '__main__':
    ...
    # specify loss
    # loss_fn = 'mean_squared_error'   # l2 loss for reconstruction. no need to conduct flatten on the label (y image)
    # loss_fn = 'binary_crossentropy'  # loss of making decisions, per element (pixel)  # for model_index 0, 1, 2
    # loss_fn = weighted_binary_crossentropy_v0  # for model_index 3
    loss_fn = soft_dice_loss           # for model_index 4
